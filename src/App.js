...
function App() {
	...
	const saveEvent = useCallback(async () => {
		...
		const startDate =
			tempEvent.start instanceof Date
				? formatDateToLocalString(tempEvent.start)
				: tempEvent.start;
		const endDate =
			tempEvent.end instanceof Date
				? formatDateToLocalString(tempEvent.end)
				: tempEvent.end;

		// 나머지 함수 내용은 그대로 유지됩니다.
		const selectedWorkerInfo = selectedWorker.map((workerId) => {
			const worker = myData.find(
				(data) => data.value === workerId
			);
			return {
				id: workerId,
	...
	const getWorkerById = async (workerId) => {
		const workerDoc = await getDoc(
			doc(db, "workerCollection", workerId)
		);
	...
	const handleWorkerChange = useCallback((ev) => {
		const selectedValue = Array.isArray(ev.value)
			? ev.value
			: [ev.value];
	...
	const changeColor = useCallback(
		(ev) => {
			const selectedColor =
				ev.currentTarget.getAttribute("data-value"); // 선택된 컬러 가져오기
	...
	return (
		<>
			...
			>
				...
				>
					<div className="event-color-label">
						Color
					</div>
					<div
